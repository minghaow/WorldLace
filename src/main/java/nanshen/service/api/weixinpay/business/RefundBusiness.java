...
import nanshen.service.api.weixinpay.protocol.refund_protocol.RefundResData;
import nanshen.service.api.weixinpay.service.RefundService;
...
public class RefundBusiness {
    ...
    public void run(RefundReqData refundReqData,ResultListener resultListener) throws Exception {
        ...
        LogUtils.info(refundServiceResponseString);
        ...
        if (refundResData == null || refundResData.getReturn_code() == null) {
            setResult2("Case1:退款API请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问");
        ...
        if (refundResData.getReturn_code().equals("FAIL")) {
            ...
            if (refundResData.getResult_code().equals("FAIL")) {
                LogUtils.info("出错，错误码：" + refundResData.getErr_code() + "     错误信息：" + refundResData.getErr_code_des());
                setResult2("Case4:【退款失败】");
                //退款失败时再怎么延时查询退款状态都没有意义，这个时间建议要么再手动重试一次，依然失败的话请走投诉渠道进行投诉
                resultListener.onRefundFail(refundResData);
            } else {
                //退款成功
                setResult2("Case5:【退款成功】");
