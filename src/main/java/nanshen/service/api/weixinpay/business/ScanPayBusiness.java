...
import nanshen.service.api.weixinpay.protocol.pay_protocol.ScanPayReqData;
import nanshen.service.api.weixinpay.protocol.pay_protocol.ScanPayResData;
import nanshen.service.api.weixinpay.protocol.pay_query_protocol.ScanPayQueryReqData;
import nanshen.service.api.weixinpay.protocol.pay_query_protocol.ScanPayQueryResData;
import nanshen.service.api.weixinpay.protocol.reverse_protocol.ReverseReqData;
import nanshen.service.api.weixinpay.protocol.reverse_protocol.ReverseResData;
import nanshen.service.api.weixinpay.service.ReverseService;
import nanshen.service.api.weixinpay.service.ScanPayQueryService;
import nanshen.service.api.weixinpay.service.ScanPayService;
...
public class ScanPayBusiness {
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        //打印回包数据
        LogUtils.info(payServiceResponseString);
        ...
        if (Configure.isUseThreadToDoReport()) {
            ...
            LogUtils.info("pay+report总耗时（异步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        } else {
            ReportService.request(reportReqData);
            timeAfterReport = System.currentTimeMillis();
            LogUtils.info("pay+report总耗时（同步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        }

        if (scanPayResData == null || scanPayResData.getReturn_code() == null) {
            LogUtils.info("【支付失败】支付请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问");
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                LogUtils.info("【一次性支付成功】");
                resultListener.onSuccess(scanPayResData);
            }else{

                //出现业务错误
                LogUtils.info("业务返回失败");
                LogUtils.info("err_code:" + errorCode);
                LogUtils.info("err_code_des:" + errorCodeDes);
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    if (errorCode.equals("AUTHCODEEXPIRE")) {
                        ...
                        LogUtils.info("【支付扣款明确失败】原因是：" + errorCodeDes);
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo)) {
                        LogUtils.info("【需要用户输入密码、查询到支付成功】");
                        resultListener.onSuccess(scanPayResData);
                    } else {
                        LogUtils.info("【需要用户输入密码、在一定时间内没有查询到支付成功、走撤销流程】");
                        doReverseLoop(outTradeNo);
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo)) {
                        LogUtils.info("【支付扣款未知失败、查询到支付成功】");
                        resultListener.onSuccess(scanPayResData);
                    } else {
                        LogUtils.info("【支付扣款未知失败、在一定时间内没有查询到支付成功、走撤销流程】");
                        doReverseLoop(outTradeNo);
                        resultListener.onFail(scanPayResData);
                    }
                }
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        LogUtils.info("支付订单查询API返回的数据如下：");
        LogUtils.info(payQueryServiceResponseString);
        ...
        if (scanPayQueryResData == null || scanPayQueryResData.getReturn_code() == null) {
            LogUtils.info("支付订单查询请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }

        if (scanPayQueryResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            LogUtils.info("支付订单查询API系统返回失败，失败信息为：" + scanPayQueryResData.getReturn_msg());
            return false;
        } else {
            if (scanPayQueryResData.getResult_code().equals("SUCCESS")) {//业务层成功
                if (scanPayQueryResData.getTrade_state().equals("SUCCESS")) {
                    //表示查单结果为“支付成功”
                    LogUtils.info("查询到订单支付成功");
                    return true;
                } else {
                    //支付不成功
                    LogUtils.info("查询到订单支付不成功");
                    return false;
                }
            } else {
                LogUtils.info("查询出错，错误码：" + scanPayQueryResData.getErr_code() + "     错误信息：" + scanPayQueryResData.getErr_code_des());
                return false;
            }
        }
    }
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
        ...
        LogUtils.info("撤销API返回的数据如下：");
        LogUtils.info(reverseResponseString);
        //将从API返回的XML数据映射到Java对象
        ...
        if (reverseResData == null) {
            LogUtils.info("支付订单撤销请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }
        if (reverseResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            LogUtils.info("支付订单撤销API系统返回失败，失败信息为：" + reverseResData.getReturn_msg());
            return false;
        } else {
            if (reverseResData.getResult_code().equals("FAIL")) {
                LogUtils.info("撤销出错，错误码：" + reverseResData.getErr_code() + "     错误信息：" + reverseResData.getErr_code_des());
